...
<template>
  <section class="mx-auto max-w-6xl space-y-6 px-6 py-10">
    ...
    <SectionCard title="參與賽事" description="api/teams/{id}/tournaments 取得參賽紀錄。">
      <LoadingState v-if="tournamentsQuery.isLoading" message="載入參賽紀錄..." />
      <p v-else-if="tournamentsQuery.error" class="text-sm text-red-600">無法取得紀錄。</p>
      <EmptyState v-else-if="!tournamentsQuery.data?.data?.length" title="尚無參賽紀錄" />
      <ul v-else class="space-y-2 text-sm text-slate-800">
        <li
          v-for="tournament in tournamentsQuery.data.data"
          :key="tournament.id"
    ...
    >
      <LoadingState v-if="invitationsQuery.isLoading" message="載入邀請..." />
      <p v-else-if="invitationsQuery.error" class="text-sm text-red-600">無法取得邀請。</p>
      <EmptyState v-else-if="!invitationsQuery.data?.data?.length" title="沒有待處理邀請" />
      <ul v-else class="space-y-2 text-sm text-slate-800">
        <li
          v-for="invitation in invitationsQuery.data.data"
          :key="invitation.id"
